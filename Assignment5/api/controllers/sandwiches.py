...
# Note: You likely need to import schemas from the 'models' package here
from ..models import models, schemas 


def create(db: Session, sandwich):
    # Create a new instance of the Sandwich model with the provided data
    # Field names used: sandwich_name and price (from your models.py)
...
def update(db: Session, sandwich_id, sandwich):
    ...
    # Check if the sandwich exists before updating
...
def delete(db: Session, sandwich_id):
    ...
    db_sandwich = db.query(models.Sandwich).filter(models.Sandwich.id == sandwich_id)
    # Check if the sandwich exists before deleting
    if not db_sandwich.first():
        raise HTTPException(status_code=404, detail="Sandwich not found")
